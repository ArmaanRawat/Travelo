...
// Removed: import stylesPaymentHistory from "../style_sheets/Payment.module.css";
...
const PaymentHistory = (props) => {
  ...
  const retrieveData = () => {
    axios
      .get(`http://localhost:8070/payments/history/all/${props.userId}`)
      .then((response) => {
  ...
  const filterContent = (_history, searchTerm) => {
    const results = _history.filter((data) =>
      data.service_name.toLowerCase().includes(searchTerm)
    );
    setData(results);
  };
      ...
      <div className={stylesPaymentHistory.container}>
        <div className={`text-center ${stylesPaymentHistory.searchContainer}`}>
          <input
            ...
            type="search"
            name="searchTerm"
            className={stylesPaymentHistory.search}
            onChange={handleTextSearch}
          />
          <button className={stylesPaymentHistory.searchBtn}>Search</button>
        </div>
        <div className={`text-center ${stylesPaymentHistory.tableContainer}`}>
          <table className={`${stylesPaymentHistory.table}`}>
            <thead className={stylesPaymentHistory.tableHead}>
              <tr>
                <th className={stylesPaymentHistory.tableContent} scope="col">
                  Order ID
                </th>
                <th className={stylesPaymentHistory.tableContent} scope="col">
                  Service Name
            ...
            {data.length !== 0 ? (
              data.map((data) => (
                <tbody>
                  <tr>
                    <td className={stylesPaymentHistory.tableContent}>
                      {data._id}
                    </td>
                    <td className={stylesPaymentHistory.tableContent}>
                      {data.service_name}
              ...
              <tbody>
                <tr>
                  <td style={{ paddingTop: "30px", paddingLeft: "10px" }}>
                    <h5>No Results</h5>
                  </td>
                </tr>
              </tbody>
